...
simulation_time_ns = 1.0 ; freq_charge_update_fs = 200 ; freq_traj_output_ps = 10 ; freq_checkpoint_ps = 10 ; write_charges = False; freq_lambda_derivative_fs = 100*freq_charge_update_fs
...
print ('temperature_step',temperature_step)
print ('temp range',np.arange(start_temp,target_temp-temperature_step,-1*temperature_step))

for cur_temperature in np.arange(start_temp,target_temp-temperature_step,-1*temperature_step):

    print ('setting temperature:',cur_temperature)
    MMsys_neutral.integrator.setTemperature(cur_temperature)
